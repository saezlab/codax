...
from codax.nn_cno.misc.profiler import do_profile
...
class CNOGraph(nx.DiGraph):
    ...
    **degree**. You can then call plot as follows to replace the default
    ...
    def draw(self, prog="dot", attribute="fillcolor",  hold=False, **kargs):
        ...
        node_colors = [self._node[x][attribute] if attribute in list(self._node[x].keys())
    ...
    def plot(self, prog="dot", viewer="pylab", hold=False,
        ...
        if node_attribute is not None:
            ...
            data = [node[1][node_attribute] for node in self.nodes(data=True)
                    if node_attribute in list(node[1].keys())]
            ...
            for node in self.nodes():
                # default
                self._node[node]['fillcolor'] = "#FFFFFF"
                try:
                    value = self._node[node][node_attribute]/float(M)
                    rgb = sm.to_rgba(value)
                    colorHex = matplotlib.colors.rgb2hex(rgb)
                    self._node[node]['fillcolor'] = colorHex
                except:
                    try:
                        color = self._node[node][node_attribute]
                        self._node[node]['fillcolor'] = colormap.Color(color).hex
                    except:
                        pass
        ...
        if node_attribute is None:
            for node in self.nodes():
                if node in self.signals and node in self.inhibitors:
                    self._node[node]['style'] = "diagonals,filled"
                    self._node[node]['color'] = "red"
                    self._node[node]['fillcolor'] = "lightblue"
                if node in self.stimuli and node in self.inhibitors:
                    self._node[node]['style'] = "diagonals,filled"
                    self._node[node]['color'] = "red"
                    self._node[node]['fillcolor'] = "#9ACD32"
        ...
        while count < 10:
            ...
            try:
                if filename.endswith('svg') and 'dpi' in list(H.graph_attr.keys()):
    ...
    def _plot_rmse_fromR(self, filename, F=.3, scale=2, col=None):
        ...
        for this in self.nodes():
            if this in self.signals:
                ...
                self._node[this]['mse'] =  (1-(mse/F)**scale)
                self._node[this]['label'] =  this+"\n"+str(int(mse*1000)/1000.)
            else:
                self._node[this]['mse'] = 1
    ...
    def _get_ranked_agraph(self, rank_method=None):
        ...
        for k, v in list(self.graph_options['graph'].items()):
            H.graph_attr[k] = v
        for k, v in list(self.graph_options['edge'].items()):
            H.edge_attr[k] = v
        for k, v in list(self.graph_options['node'].items()):
            H.node_attr[k] = v
        ...
        ranks  = {}
        M = max(allranks.keys())
        for k, v in list(allranks.items()):
        ...
        selfloops = [x[1] for x in nx.selfloop_edges(self)]
    ...
    def _get_and_attribute(self, reaction):
        ...
        preds = list(self.predecessors(reaction))
        ...
        for node in ands_before:
            ...
            if len(succs) == 1:
                ...
                if newname != r1.name:
                    ...
                    self._node[newname.name].update(**attr2)
                    self._node[newname.name].update(**attr)
                    self.remove_node(node)
            else:
    ...
    def set_node_attributes(self, attr_dict ):
        ...
        for node in list(attr_dict.keys()):
            if node in self.nodes():
                for k,v in list(attr_dict[node].items()):
                    self._node[node][k] = v
    ...
    def set_edge_attribute(self, name, values):
        ...
        for reaction in list(values.keys()):
            if reaction in list(values.keys()):
                edges = self.reac2edges(reaction)
                for edge in edges:
                    e0, e1 = edge[0], edge[1]
                    self.edges[e0,e1][name] = values[reaction]
    ...
    def _get_inputs(self):
        return [x for x in self.nodes() if len(list(self.predecessors(x)))==0]
    ...
    def get_max_rank(self):
        ...
        ranks = [[x for x in list(func_path[stimulus].values()) if x !=pylab.inf]
    ...
    def expand_or_gates(self):
        ...
        for this in self._find_and_nodes():
            p = list(self.predecessors(this))
            s = list(self.successors(this))
            assert len(s) == 1
            for node in p:
    ...
    def _ambiguous_multiedge(self, node):
        ...
        for A in list(self.predecessors(node)):
            for B in list(self.successors(node)):
    ...
    def relabel_nodes(self, mapping, inplace=True):
        ...
        edge_data = [x for x in self.edges(data=True) if x[0] in list(mapping.keys())
            or x[1] in list(mapping.keys())]
        ...
        for edge in edge_data:
            ...
            if "=" in edge[0]:  # an AND gate
                ...
                e0 = r.name
            elif edge[0] in list(mapping.keys()):
                e0 = mapping[edge[0]]
            else:
                # normal node
                e0 = edge[0]
            ...
            if "=" in edge[1]:  # an AND gate
                ...
                r.rename_species(mapping)
                e1 = r.name
            elif edge[1] in list(mapping.keys()):
                e1 = mapping[edge[1]]
            else:
                e1 = edge[1]
        ...
        # defined names
        self._stimuli = [mapping[x] if x in list(mapping.keys()) else x for x in self._stimuli]
        self._signals = [mapping[x] if x in list(mapping.keys()) else x for x in self._signals]
        self._inhibitors = [mapping[x] if x in list(mapping.keys()) else x for x in self._inhibitors]
        self._compressed = [mapping[x] if x in list(mapping.keys()) else x for x in self._compressed]

        # copy the node data
        for k, v in list(mapping.items()):
            self._node[v] = self._node[k]
        ...
        self.remove_nodes_from(list(mapping.keys()))
        self._changed = True
    ...
    def centrality_eigenvector(self, max_iter=1000, tol=0.1):
        ...
        import operator
        degcent_sorted = sorted(list(res.items()), key=operator.itemgetter(1), reverse=True)
    ...
    def merge_nodes(self, nodes, node):
        ...
        self.add_node(node)
        for n in nodes:
            for pred in list(self.predecessors(n)):
    ...
    def split_node(self, node, nodes):
        ...
        """
        for n in nodes:
            for pred in list(self.predecessors(node)):
            ...
            for succ in list(self.successors(node)):
...
class AGraphCNO(gv.AGraph):
    ...
    def to_dot(self, filename=None, frmt='png'):
        ...
        txt = "digraph G{\n"
        for k, v in list(self.cnograph.graph_options['graph'].items()):
