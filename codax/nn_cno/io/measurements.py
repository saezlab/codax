...
class Measurement(object):
    ...
    def _set_inhibitors(self, inhibitors):
        ...
        for k,v in list(inhibitors.items()):
            if v >1 or v<0:
...
class Measurements(list):
    ...
    def _get_stimuli(self):
        stimuli = set()
        for e in self:
            stimuli = stimuli.union(list(e.stimuli.keys()))
        return stimuli
    ...
    def _get_inhibitors(self):
        inhibitors = set()
        for e in self:
            inhibitors = inhibitors.union(list(e.inhibitors.keys()))
        return inhibitors
